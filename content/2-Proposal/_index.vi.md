...
### 6. Phân tích Chi phí Chi tiết & ROI (Chính xác theo 24 Nút Dịch vụ trên Diagram)

Dựa trên kiểm đếm chính xác **24 nút tài nguyên (resource nodes)** thuộc **22 dịch vụ AWS** xuất hiện trên sơ đồ kiến trúc `platform_architecture.drawio`, dưới đây là bảng tính toán chi phí vận hành hàng tháng chi tiết:

#### Bảng Chi Tiết Chi Phí Theo Đúng 24 Nút Trên Sơ Đồ Kiến Trúc (Quy mô Standard: 100 Tenants ~ 5,000 Nhân Viên, 3M Requests/Tháng)

| STT | Dịch Vụ AWS Trên Diagram | Số Lượng Nút (Instances) | Thông Số & Đơn Giá Sử Dụng (Hàng Tháng) | Chi Phí (USD/Tháng) |
| :---: | :--- | :---: | :--- | :--- |
...
| **3** | **AWS Shield** | 1 Node | Layer 3/4 DDoS Protection (Miễn phí đi kèm CloudFront) | **$0.00** |
| **4** | **Amazon CloudFront** | 1 Node | 50GB Data Transfer Out + 3M HTTPS Req (Free Tier 1TB) | **$0.00** |
| **5** | **Amazon S3 (SPA Hosting)** | 1 Node | 1GB React Static Assets + 100k GET Requests | **$0.06** |
| **6** | **Amazon Cognito** | 1 Node | 5,000 MAUs User Auth (Free Tier hỗ trợ đến 50,000 MAUs) | **$0.00** |
...
| **11**| **Amazon SQS (Report Queue)** | 1 Node | 20,000 Report Queue Messages (Free Tier 1,000,000 SQS Req) | **$0.00** |
| **12**| **Amazon SQS (Email Queue)** | 1 Node | 20,000 Email Queue Messages (Free Tier 1,000,000 SQS Req) | **$0.00** |
| **13**| **Amazon DynamoDB** | 1 Node | Single-Table: 3M Writes ($3.75) + 6M Reads ($1.50) + 5GB ($1.25) | **$6.50** |
| **14**| **DynamoDB Streams** | 1 Node | CDC Event Streams (2.5M stream requests miễn phí) | **$0.00** |
...
| **21**| **AWS CloudFormation** | 1 Node | Infrastructure as Code Resource Management (Miễn phí) | **$0.00** |
...
| **TỔNG CHI PHÍ HẠ TẦNG**| **24 Nodes** | **Đầy đủ 24 nút dịch vụ trên Diagram (Full Protection & Tracing)** | **~$35.51 USD/tháng** |

> [!NOTE]
> **Tối ưu chi phí giai đoạn khởi chạy (Basic Profile):** Nếu tắt WAF nâng cao và X-Ray tracing giai đoạn đầu, tổng chi phí hạ tầng thực tế của toàn bộ 24 nút Serverless chỉ rơi vào khoảng **~$23.21 USD/tháng**.
...
+----------------------------+----------------------------------+--------------------+----------------+
| Starter (10 Tenants)       | 500 Staff · 300K Requests/tháng  | ~$4.20 USD/tháng   | ~$85.00 USD    |
| Standard (100 Tenants)     | 5,000 Staff · 3M Requests/tháng  | ~$35.51 USD/tháng  | ~$165.00 USD   |
| Enterprise (1,000 Tenants) | 50,000 Staff · 30M Requests/tháng| ~$156.80 USD/tháng | ~$650.00 USD   |
+---------------------------------------------------------------------------------------------------+
```

> [!TIP]
> **Tỷ lệ Hoàn vốn (ROI):**
> * **Tiết kiệm đến 78% - 95%** chi phí hạ tầng hàng tháng so with mô hình máy chủ EC2 + RDS duy trì 24/7.
